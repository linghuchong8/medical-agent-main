...
def _log_embed_error(kind: str, exc: Exception, attempt: int) -> None:
    """记录 embedding 失败详情，便于定位 Ollama/网络问题。"""
    status = getattr(exc, "status_code", None)
    detail = getattr(exc, "error", None)
    logger.error(
        f"embedding 失败({kind}, 第{attempt + 1}次): "
        f"type={type(exc).__name__} status={status} detail={detail!r} err={exc}"
    )
...
class MilvusStore(BaseStore):
    ...
    def _embed_text(self, text: str) -> list[float]:
        ...
        for attempt in range(self._MAX_EMBED_RETRIES):
            try:
                return self._embeddings.embed_query(text)
            except Exception as e:
                last_exc = e
                _log_embed_error("sync", e, attempt)
    ...
    async def _aembed_text(self, text: str) -> list[float]:
        ...
        for attempt in range(self._MAX_EMBED_RETRIES):
            try:
                ...
                last_exc = e
                _log_embed_error("async", e, attempt)
